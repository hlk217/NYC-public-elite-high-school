...
navbarPage("NYCpass", id="nav",
  ...
  tabPanel("Interactive map",
    div(class="outer",
      ...
      absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
        ...
        checkboxInput("addmarker", "Add marker on targetted school to promote diversity based on the following criteria", FALSE),
        sliderInput('predRange', 'Predicted Offer Rate Range', min=min(nycSchool$predictedOfferRatio, na.rm=T), max=max(nycSchool$predictedOfferRatio, na.rm=T),
                    value=c( as.numeric( format(round(max(nycSchool$predictedOfferRatio, na.rm=T), 2), nsmall = 2))  -0.1 , as.numeric( format(round(max(nycSchool$predictedOfferRatio, na.rm=T), 2), nsmall = 2) )  ), step=0.1, round=0),
        h6("School's major ethnicity group "),
        checkboxInput("black", "Black", FALSE),
        checkboxInput("hispanic", "Hispanic", FALSE),
        checkboxInput("white", "White", FALSE),
  ...
  # include the data analysis based on md generated by rmd
